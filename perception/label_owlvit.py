...
import torch
import numpy as np
from perception.label import Label
...
class LabelOWLViT(Label):
    ...
    def get_boxes(self, input_image, text_queries, scores, boxes, labels):
        ...
        for score, box, label in zip(scores, boxes, labels):
            if score < self.SCORE_THRESHOLD:
                continue
            pbox = self.xywh_to_x1y1x2y2(box)
    ...
    def get_preds(self, outputs, target_sizes):
        ...
        self.sorted_boxes_coords = np.array([self.xywh_to_x1y1x2y2(box) for box in boxes[self.sorted_indices]])
